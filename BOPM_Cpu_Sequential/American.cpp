...
int main() {
	...
	for (int i = 0; i < step; i++) {
		node[time[step] + i].option_price = exercise_price - node[time[step] + i].stock_price;
	...
	for (int i = step - 1; i >=1; i--) {
		for (int j = 0; j < i; j++) {
			...
			if (node[time[i] + j].option_price < -node[time[i] + j].stock_price + exercise_price) node[time[i] + j].option_price = -node[time[i] + j].stock_price + exercise_price;
		}
	}
